...
\author{Steven W. Jones\thanks{stevenwarejones@gmail.com. Drafted with substantial assistance from Claude (Anthropic) and Codex (OpenAI); see the Statement on AI use. \textbf{Draft v1.10 --- machine-verified but not yet independently checked by a domain expert. Not for submission in current form.}}}
\date{August 15, 2026}
...
\label{obs:mech}
Along the tilted-cluster family (controlled-phase $\theta\pi$), the identity $\Sig = \max\{0,(S_4-6)/8\}$ holds to $9$ digits at $\theta \in \{0.85, 0.9, 0.95, 1\}$, including the zero crossing; and the optimal LP dual at the cluster point, evaluated as an affine functional on independent tilted states, reproduces $(S_4-6)/8$ exactly. The $S_4$ facet with slope $8$ is the unique active constraint on this family.
\end{observation}
...
The vertex check is exhaustive and finite, hence a proof; the statement holds for all no-signaling behaviors, not merely quantum ones. (The positivity argument was contributed in an automated referee round; see the Statement on AI use. Independent numerical confirmation: on $400$ random quantum behaviors the LP-computed constrained distance equals $(S-2)/8$ to $9$ digits.)
...
\begin{table}[t]
\centering
\caption{Adversarial record of Conjecture~\ref{conj:tight}. All values are floating-point LP results; ``saturates'' means numerical agreement with $(\sqrt2-1)/4=0.103553390593\ldots$ to at least $9$ digits ($12$ for the parallel-composition row). Only the LC4 row carries an exact certificate (Theorem~\ref{thm:exact}); solver feasibility tolerance is $10^{-9}$.}
\label{tab:attacks}

...
\]
Define $\Dacc(P;G)$ as the largest total-variation distance, over $\mathcal A_G$ and over settings of the parties outside $\{X\}\cup R$ held fixed in advance, between the two distributions of $R$'s outcomes induced by the two settings of $X$; the maximum over an empty family is $0$. Only $A$ and $D$ appear as senders because the blind pair cannot signal to its complement in this model class. This quantifies a \emph{single-trial, direct record-collection} task: no ancillas, no adaptivity, no repetition, and no access to hidden variables.
...
Both exceptions are live rather than vacuous: a compatible model can put a signal of size $0.323$ into $\{B,C\}$ and $0.213$ into $\{B,C,D\}$ (LP maxima). Write $\CBC$ for the condition that $\{B,C\}$ is collectible for either early sender, and $\CFull$ for the condition that both full complementary records are collectible.
...
0, & \text{if neither } \CFull \text{ nor } \CBC.
\end{cases}
\]
Moreover, when neither holds the value is $0$ for \emph{every} compatible $P$, not merely at the minimum.
\end{proposition}

\noindent\emph{Proof.} Under $\CFull$, total variation cannot increase under marginalization, so $\Dacc=\max\{\delta_A,\delta_D\}$ and Theorem~\ref{thm:exact} gives the value. If neither condition holds then by Lemma~\ref{lem:pinned} every collectible $R$ is pinned, so every compatible $P$ has $\Dacc=0$; the minimum is attained trivially, and \texttt{A\_only}, \texttt{D\_only} of \texttt{verify\_invisibility.py} exhibit models that additionally saturate $\Sig$ while showing nothing to any proper subset. $\square$
...
\emph{Relation to the jamming literature.} The distinction between correlations that are signaling in the abstract and correlations that deliver a usable message is not new: it is the substance of the relativistic-causality-versus-no-signaling analysis of Ref.~\cite{HorodeckiRamanathan2019}, where geometry decides which multipartite influences are admissible, and of the monogamy constraints on relativistically causal correlations in Ref.~\cite{Weilenmann2024}; Ref.~\cite{Ramanathan2026} studies jamming extensions that retain a quantum underlying state and obtains hidden-superluminal-signaling results with cryptographic applications. What is specific here is the exact criterion for $Q_{LC4}$ within the fixed conditionally-local completion class, under the single-trial direct-collection restrictions above, together with attaining models; those extension assumptions differ from ours, and nothing here refutes or is refuted by them.
...
\bibitem{Weilenmann2024} M. Weilenmann \emph{et al.}, Nat. Commun. \textbf{15}, 10442 (2024).
\bibitem{Ramanathan2026} R. Ramanathan, Y. Xie, M. Eckstein, and P. Horodecki, arXiv:2608.20247 (2026).

...
Both exceptions are live rather than vacuous: over compatible models the largest achievable difference in a \emph{single} outcome probability is $0.323$ for $\{B,C\}$ and $0.213$ for $\{B,C,D\}$ (LP maxima). These are single-outcome differences, not total-variation distances---since $\mathrm{TV}\ge\frac12\max_o|\Delta p(o)|$ they certify $\mathrm{TV}\ge0.162$ and $0.107$ respectively, and marginalization gives the consistent ordering $\mathrm{TV}_{BCD}\ge \mathrm{TV}_{BC}$. Write $\CBC$ for the condition that $\{B,C\}$ is collectible for either early sender, and $\CFull$ for the condition that both full complementary records are collectible.

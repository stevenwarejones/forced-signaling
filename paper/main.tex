...
Both exceptions are live rather than vacuous: over compatible models the largest achievable difference in a \emph{single} outcome probability is $0.323$ for $\{B,C\}$ and $0.213$ for $\{B,C,D\}$ (LP maxima). These are single-outcome differences, not total-variation distances; but a singleton is an event, and $\mathrm{TV}=\max_S|p(S)-q(S)|$, so each certifies a compatible model with $\mathrm{TV}$ at least $0.323$ and $0.213$ respectively. (The two come from separate optimizations, so their order carries no information: marginalizing $BCD$ to $BC$ sums over $d$ and can enlarge a single-outcome difference. The \emph{maxima} of $\mathrm{TV}$ do obey $\max\mathrm{TV}_{BCD}\ge\max\mathrm{TV}_{BC}\ge0.323$, since $\mathrm{TV}$ cannot increase under marginalization within a fixed model.) Write $\CBC$ for the condition that $\{B,C\}$ is collectible for either early sender, and $\CFull$ for the condition that both full complementary records are collectible.

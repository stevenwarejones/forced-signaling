...
\author{Steven W. Jones\thanks{stevenwarejones@gmail.com. Drafted with substantial assistance from Claude (Anthropic) and Codex (OpenAI); see the Statement on AI use. \textbf{Draft v1.12 --- machine-verified but not yet independently checked by a domain expert. Not for submission in current form.}}}
\date{August 15, 2026}
...
The vertex check is exhaustive and finite, hence a proof; the statement holds for all no-signaling behaviors, not merely quantum ones. (The positivity argument was contributed in an automated referee round; see the Statement on AI use. Independent numerical confirmation: on $400$ random quantum behaviors the LP-computed constrained distance agrees with $(S-2)/8$ to a maximum observed deviation of $2.5\times10^{-16}$; this is observed agreement of a floating-point solve, not a certified error bound.)

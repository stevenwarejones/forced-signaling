...
\newcommand{\CBC}{\mathrm{C}_{BC}}
\newcommand{\CFull}{\mathrm{C}_{\mathrm{full}}}
...
Operationally, the inequality being tested is Proposition~\ref{prop:directional}(i) itself: the class is rejected when
\begin{equation}
S_4^{\rm op} \;>\; 6 + 4\delta_A + 4\delta_D ,
\label{eq:dirtest}
\end{equation}
so with a jointly valid lower confidence bound $L_S$ on the witness and upper bounds $U_A,U_D$ on the two directional distances, a sufficient rejection condition is $L_S > 6 + 4U_A + 4U_D$. Small signaling alone rejects nothing---the uniform distribution is conditionally local with zero signaling---and the fixed threshold $\delta_A+\delta_D<(\sqrt2-1)/2$ is the specialization of \eqref{eq:dirtest} to the \emph{ideal} target, where $S_4^{\rm op}=4+2\sqrt2$. Read that way the refinement can succeed where the scalar test $\Dsig<(\sqrt2-1)/4$ fails: slack below the threshold on one side may be spent above it on the other, so ideal-target bounds $(\delta_A,\delta_D)=(0.100,0.105)$ suffice although $\max=0.105$ exceeds $(\sqrt2-1)/4$; as a measured test the same pair requires $L_S>6.820$. The directional form needs \emph{simultaneous} valid bounds on two quantities rather than one bound on their maximum, though we do not claim this is necessarily more expensive---estimating a maximum already ranges over directions and contexts, and the combined witness can also be bounded directly. Which form is cheaper depends on the inference procedure and should be settled at design time.
...
\begin{observation}[Support of the optimal dual]
\label{obs:dualsupport}
The $\Sig$ program imposes sixteen signaling comparisons---eight from changing $A$'s setting, eight from changing $D$'s. Every nonzero entry of the certified dual of Theorem~\ref{thm:exact} lies in just four of them: the $A$-setting changes at $w=1$ with $z=1$ (both values of $y$), and the $D$-setting changes at $x=1$ with $z=0$ (both values of $y$), and all $36$ of its nonzero entries are equal, so the inequality dual is $\tfrac18$ times a $0/1$ vector. The consequent reduction is two-sided, and both sides are already certified: deleting the twelve remaining total-variation constraints \emph{enlarges} the primal feasible set, so the exhibited model stays feasible there and bounds the restricted optimum above by $(\sqrt2-1)/4$, while the dual carries no weight on the deleted rows and so stays dual feasible for the reduced program with the same objective, bounding it below by the same value. Hence the restriction of $\Sig$ to those four comparisons equals $\Sig$ over all sixteen \emph{exactly} at the cluster point; both equal $(\sqrt2-1)/4$. The support and the coefficients are checked in exact arithmetic by the accompanying verifier. The restriction is moreover not special to the exhibited dual, and follows from optimality rather than from feasibility: the dual with all equality multipliers zero and $1/8$ on a single one of the twelve remaining total-variation rows is feasible, with objective $0$. But in the certified primal each of those twelve rows is slack by exactly $(\sqrt2-1)/2$ while the four are tight, so complementary slackness zeroes their multipliers in \emph{every} optimal dual, and each auxiliary column couples as $\lambda'+\lambda''\le\lambda^{\rm TV}$, so a vanishing total-variation multiplier zeroes both absolute-value multipliers for each of its outcomes as well. Hence every optimal dual is supported within the same four comparisons---though not necessarily with the same support or the same coefficients, and not demonstrably so at any other behavior. Two limits are worth stating. This identifies which constraints are active at $Q_{LC4}$---it is the content of complementary slackness, not a new bound---and in particular it is not an upper bound valid at other behaviors: deleting constraints can only decrease $\Sig$, so an \emph{upper} bound proved for the four-comparison restriction does not transfer to the full program. Lower bounds do transfer, the restricted optimum never exceeding the full one. Nor is it shown here that the optimal dual has this support for behaviors other than the cluster point.
\end{observation}
...
In the arrangement $A \prec_v D \prec_v B,C$, $B\sim C$, the early-side relations are $v$-cone \emph{inclusions} with enormous slack; only the blind-pair \emph{exclusion} is metrologically critical: blind stations separated by distance $d$ must measure within $d/(\kappa c)$ in the preferred frame ($3.3$\,ns at $d=10$\,km, $v=10^4c$). Consequently the early-side measurements may be co-located in the source laboratory with microsecond staggering (margin $>20\times$ for all $|\beta|\le1.3\times10^{-3}$), reducing the required infrastructure to \emph{three} sites---one source laboratory and two remote stations, the topology of existing two-party Bell experiments \cite{Salart2008}. The trade-off is explicit, and sharper than a weakening: the five-separated-site arrangement of Ref.~\cite{Li2026} additionally supports collection events that render a surviving model's hidden signaling into explicit communication protocols, whereas co-location \emph{removes} that layer entirely, by Proposition~\ref{prop:access} below. The statistical exclusion test is untouched---it is a test on marginals, not a protocol---but the three-site layout must not be described as realizing a superluminal channel: with the early parties between the stations, \emph{no} compatible model gives an accessible signal there. A four-site variant that restores it, at the cost of one station, is given after Proposition~\ref{prop:access}. Figure~\ref{fig:geometry} shows the geometry.

Write $J_c^+(K)$ for the causal future of a measurement event $K$, and fix a layout $G$ in which every $K_P$ is placed independently of the setting chosen there, with records propagating no faster than $c$. Say a nonempty recipient set $R\subseteq\{A,B,C,D\}\setminus\{X\}$ is \emph{$X$-collectible} if
\[
\Big(\bigcap_{Y\in R}J_c^+(K_Y)\Big)\setminus J_c^+(K_X)\;\neq\;\varnothing,
\]
i.e.\ if some event receives every outcome in $R$ before light from $X$ arrives, and let
\[
\mathcal A_G=\big\{(X,R): X\in\{A,D\},\ R \text{ is } X\text{-collectible}\big\}.
\]
Define $\Dacc(P;G)$ as the largest total-variation distance between the two distributions of $R$'s outcomes induced by the two settings of $X$, maximized over $\mathcal A_G$ and over every setting of every party other than $X$---including the recipients' own settings---each held fixed in advance for the trial; the maximum over an empty family is $0$. Only $A$ and $D$ appear as senders because the blind pair cannot signal to its complement in this model class. This quantifies a \emph{single-trial, direct record-collection} task: no ancillas, no adaptivity, no repetition, and no access to hidden variables.

\begin{lemma}[Only the blind pair can carry a signal]
\label{lem:pinned}
Let $P$ be conditionally local and reproduce every no-blind-pair marginal of $Q_{LC4}$. Then for sender $A$ the only recipient sets whose distribution can depend on $A$'s setting are $\{B,C\}$ and $\{B,C,D\}$; for sender $D$, only $\{B,C\}$ and $\{A,B,C\}$. Every other recipient set is \emph{pinned}.
\end{lemma}

\noindent\emph{Proof.} Any $R$ omitting $B$ or omitting $C$ has its distribution determined by marginalizing the reproduced $ABD$ or $ACD$ family, which equals the quantum one and is therefore no-signaling; so $P_R$ cannot depend on the sender's setting. The remaining sets are those containing both $B$ and $C$. $\square$

Both exceptions are live rather than vacuous: over compatible models the largest achievable difference in a \emph{single} outcome probability is $0.323$ for $\{B,C\}$ and $0.213$ for $\{B,C,D\}$ (LP maxima). These are single-outcome differences, not total-variation distances; but a singleton is an event, and $\mathrm{TV}=\max_S|p(S)-q(S)|$, so each certifies a compatible model with $\mathrm{TV}$ at least $0.323$ and $0.213$ respectively. (Their order carries no information, because a single-outcome difference---unlike $\mathrm{TV}$---is not monotone under marginalization: summing $BCD$ over $d$ can enlarge one. That the two come from separate optimizations is \emph{not} the reason; a monotone quantity keeps its ordering under separate maximization over the same class, and the $\mathrm{TV}$ maxima accordingly do obey $\max_P\mathrm{TV}_{BCD}\ge\max_P\mathrm{TV}_{BC}\ge0.323$.) Write $\CBC$ for the condition that $\{B,C\}$ is collectible for either early sender, and $\CFull$ for the condition that both full complementary records are collectible.
...
\min_P \Dacc(P;G) =
\begin{cases}
\tfrac{\sqrt2-1}{4}, & \CFull,\\[2pt]
0, & \text{otherwise.}
\end{cases}
\]
If in addition $\CBC$ fails, the value is $0$ for \emph{every} compatible $P$, not merely at the minimum.
\end{proposition}

\noindent\emph{Proof.} Under $\CFull$ both full complementary records are collectible, so $\Dacc\ge\max\{\delta_A,\delta_D\}\ge\Sig$; the \texttt{balanced} model of \texttt{verify\_invisibility.py} has $\delta_A=\delta_D=\Sig$ and shows nothing to any proper subset, so it attains the value. (The one-sided models do \emph{not} saturate $\Sig$: each has maximum directional signaling $(\sqrt2-1)/2=2\Sig$; what they attain is the minimum \emph{total} budget of Proposition~\ref{prop:directional}.) If $\CFull$ fails, say for $A$, take \texttt{A\_only}: $D$, $B$ and $C$ signal nowhere, $A$'s signal sits only in $\{B,C,D\}$, which is not collectible, and every proper subset---$\{B,C\}$ included---is blind, so $\Dacc=0$; symmetrically with \texttt{D\_only}. If moreover $\CBC$ fails then by Lemma~\ref{lem:pinned} every collectible $R$ is pinned, so $\Dacc=0$ for every compatible $P$. $\square$

Imposing pairwise invisibility therefore costs nothing: the minimum forced signaling is still $(\sqrt2-1)/4$, attained by a model in which no proper subset of recipients sees anything (exact certificates; \texttt{verify\_invisibility.py}). The consequence for Sec.~\ref{sec:experiment}'s layout is that \emph{both} conditions fail, so by Proposition~\ref{prop:access} $\Dacc=0$ for every compatible model, not merely for some. $\CFull$ fails because $A$ and $D$ are co-located with $D$ later, so $J_c^+(K_D)\subseteq J_c^+(K_A)$ and hence $\bigcap_{Y\neq A}J_c^+(K_Y)\subseteq J_c^+(K_A)$. $\CBC$ fails for a purely geometric reason, and this is where the layout's symmetry earns its keep: a collection event for $\{B,C\}$ would have to lie closer to $B$ than to the sender \emph{and} closer to $C$ than to the sender (by at least the staggering time), i.e.\ in the intersection of the two half-spaces bounded by the perpendicular bisectors. When the source laboratory lies on the segment $BC$---as it does, feeding both arms---those half-spaces are disjoint and the intersection is empty. The hypothesis is therefore explicit: \emph{the early parties lie between the two blind stations}. Move them off that segment and $\CBC$ can be recovered. \emph{Restoring the communication layer.} The defect is repaired by separating the early side, at the cost of one additional station. Place the four measurements on a $12$\,km line with the early parties \emph{outboard} and the blind pair inboard:
\begin{align*}
A&: t=0,\ x=-6\,\text{km}; & B&: t=200\,\text{ns},\ x=-5\,\text{km};\\
D&: t=100\,\text{ns},\ x=+6\,\text{km}; & C&: t=200\,\text{ns},\ x=+5\,\text{km}.
\end{align*}
All six pairs are $c$-spacelike (tightest margin $3.1\,\mu$s, for $A$--$B$), and $B\sim C$ is unchanged---the blind pair still separated by $10$\,km with the same critical $3.3$\,ns window. \emph{This example does not inherit the speed coverage of Theorem~\ref{thm:cover}, and we state its range explicitly.} The binding requirement is $A\prec_v D$: with $L=12$\,km and a $100$\,ns stagger it needs $\kappa>L/(c\tau)=400.28$ in the laboratory frame, and $\kappa>863.35$ for the worst aligned boost $|\beta|\le1.34\times10^{-3}$. The layout is therefore an example \emph{at} $v=10^4c$---where the inclusions hold with $12$--$300\times$ margin---valid down to $v\approx8.6\times10^2c$, not across the whole target region. The obstruction is structural rather than an artifact of these numbers. Within the assumed arrangement---$A\prec_v D\prec_v B,C$ with $B\sim C$, and both full complementary records collectible---every pair must be $c$-spacelike: $\CFull$ for $A$ requires $B,C,D\notin J_c^+(K_A)$ and for $D$ requires $A,B,C\notin J_c^+(K_D)$, while $B\sim C$ supplies the remaining pair. (Collectibility on its own imposes no such requirement in general.) Hence $\tau<L/c$, while the $v$-inclusion forces $\tau>L/(\kappa c)$, so a \emph{fixed} early geometry covers only $\kappa>L/(c\tau)$; lowering that threshold means enlarging $\tau$ toward $L/c$, which the accumulated staggering along $A\to D\to B,C$ then trades against the $c$-spacelike margin of the closest pair. Covering the full range would need the early stagger programmed as well, in the manner of Theorem~\ref{thm:cover}; we have not worked that out. Both complementary records are now collectible: for sender $A$, collecting at $x=+6$\,km, the $BCD$ records are complete at $36.89\,\mu$s while light from $A$ arrives at $40.03\,\mu$s (margin $3.1\,\mu$s); symmetrically for $D$ at $x=-6$\,km (margin $3.2\,\mu$s). By Proposition~\ref{prop:access} this layout therefore carries $\min\Dacc=(\sqrt2-1)/4$.

\emph{Relation to the jamming literature.} The distinction between correlations that are signaling in the abstract and correlations that deliver a usable message is not new: it is the substance of the relativistic-causality-versus-no-signaling analysis of Ref.~\cite{HorodeckiRamanathan2019}, where geometry decides which multipartite influences are admissible, and of the monogamy relations for relativistically causal correlations of Ref.~\cite{Weilenmann2024}; Ref.~\cite{Ramanathan2026} studies jamming extensions that retain a quantum underlying state and obtains hidden-superluminal-signaling results with cryptographic applications. What is specific here is the exact criterion for $Q_{LC4}$ within the fixed conditionally-local completion class, under the single-trial direct-collection restrictions above, together with attaining models; those extension assumptions differ from ours, and nothing here refutes or is refuted by them.
...
\bibitem{HorodeckiRamanathan2019} P. Horodecki and R. Ramanathan, Nat. Commun. \textbf{10}, 1701 (2019).
\bibitem{Weilenmann2024} M. Weilenmann, Nat. Commun. \textbf{16}, 269 (2025).
\bibitem{Ramanathan2026} R. Ramanathan, Xie Sicheng, M. Eckstein, and P. Horodecki, arXiv:2608.20247 (2026).
